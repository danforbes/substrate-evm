import React from 'react';
import { Button } from 'semantic-ui-react';
import { web3FromSource } from '@polkadot/extension-dapp';

import { useSubstrate } from '../';

export default function TxButton ({
  accountPair = null,
  label,
  setStatus,
  style = null,
  type = null,
  attrs = null,
  disabled = false
}) {
  const { api } = useSubstrate();
  const { params = null, sudo = false, tx = null } = attrs;
  const isQuery = () => type === 'QUERY';

  const query = async () => {
    try {
      const result = await tx(...params);
      setStatus({ code: 0, message: result.toString() });
    } catch (e) {
      console.error('ERROR query:', e);
      setStatus({ code: 1, message: e.toString() });
    }
  };

  const transaction = async () => {
    const {
      address,
      meta: { source, isInjected }
    } = accountPair;
    let fromParam;

    // set the signer
    if (isInjected) {
      const injected = await web3FromSource(source);
      fromParam = address;
      api.setSigner(injected.signer);
    } else {
      fromParam = accountPair;
    }

    let txExecute;
    try {
      // Check if tx has params
      if (!params) {
        txExecute = !sudo ? tx() : tx.sudo();
      } else {
        txExecute = !sudo ? tx(...params) : tx.sudo(...params);
      }
    } catch (e) {
      console.error('ERROR forming transaction:', e);
      setStatus({ code: 1, message: e.toString() });
    }

    if (txExecute) {
      txExecute
        .signAndSend(fromParam, ({ events = [], status }) => {
          status.isFinalized
            ? setStatus(
              { code: 0, message: `Transaction completed at block ${status.asFinalized.toString()}`, events: events }
            )
            : setStatus({ code: null, message: `Transaction status: ${status.type}` });
        })
        .catch(e => {
          setStatus({ code: 1, message: `Transaction failed: ${e.toString()}` });
          console.error('ERROR transaction:', e);
        });
    }
  };

  const onClick = () => {
    setStatus({ code: -1, message: 'Sending...' });
    isQuery() ? query() : transaction();
  };

  return (
    <Button
      primary
      style={style}
      type='submit'
      onClick={onClick}
      disabled={disabled || !tx || (!isQuery() && !accountPair)}
    >
      {label}
    </Button>
  );
}
